...
需要 Python 3.12、FFmpeg（离线 slideshow 需要包含 `drawtext`、libfreetype 和 libharfbuzz 的完整构建）。Homebrew Core 的精简 `ffmpeg` 不包含该滤镜；macOS 请安装可并存的 keg-only 完整版本：
...
- Phase 2：可恢复本地任务队列、Redis/PostgreSQL 多进程队列、幂等、重试、事件、Provider 能力、运维 API、Dashboard、S3/MinIO 产物后端、PostgreSQL 元数据目录和 Asset Library 内容生命周期已完成。
